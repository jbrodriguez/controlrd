...
const (
	ipmiBinary = "/usr/sbin/ipmisensors"
	ipmiConfig = "/boot/config/plugins/ipmi/ipmi.cfg"
)
...
func NewIpmiSensor() *IpmiSensor {
	...
	if err != nil {
		logger.Yellow("ipmi:unable to parse config:(%s):(%s). No sensors available", ipmiConfig, err)
	}
